...
def search_and_summarize() -> list[dict]:
    ...
    response = client.messages.create(
        ...
        max_tokens=2000,
        system=(
            "You are a JSON-only response bot. You must ALWAYS respond with a valid JSON array and nothing else. "
            "No prose, no markdown, no backticks, no explanation before or after. Only raw JSON."
        ),
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "\n\n"
                    "Return a JSON array of 3-6 items. Each item must have exactly these fields:\n"
                    "  - title: short headline (e.g. 'Takeda Cuts 634 Jobs in U.S. Restructuring')\n"
                    "  - summary: 1-2 sentences covering what happened, jobs affected, and reason if known\n"
                    "  - url: direct link to the source article\n"
                    "\n"
                    "Example: "
                    '[{"title": "Acme Bio Lays Off 200", "summary": "Acme Bio cut 200 jobs after a Phase III failure.", "url": "https://example.com"}]'
                ),
            }
        ],
    )

    # Debug: print all content blocks so we can see what Claude returned
    print("\n--- RAW RESPONSE BLOCKS ---")
    ...
    try:
        ...
        print(f"✅ JSON parsed successfully, {len(items)} items found")
